...
export default function DramaCategoryPage({ params }: { params: { id: string } }) {
  const id = params.id;
  ...
  async function load(p: number) {
    setLoading(true);
    const raw = await apiGet<any>(`/v1/drama/category/${id}?page=${p}&size=10`);
  ...
  return (
    <div>
      <TopBar title={`Drama • Genre: ${id}`} backHref="/drama/categories" />
