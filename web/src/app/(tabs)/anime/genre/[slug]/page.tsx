...
export default function AnimeGenrePage({ params }: { params: { slug: string } }) {
  ...
  async function load(p: number) {
    setLoading(true);
    const raw = await apiGet<any>(`/v1/anime/genre/${slug}?page=${p}`);
  ...
  return (
    <div>
      <TopBar title={`Anime • Genre: ${slug}`} backHref="/anime/genres" />
